...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'verify_otp') {
        ...
        $stmt = $conn->prepare("SELECT Email, Password, role, `First Name`, failed_attempts, lockout_time FROM employees WHERE Email = ?");
        if (!$stmt) {
        ...
        if ($stmt->num_rows == 1) {
            ...
            if ($failed_attempts < 5) { // Only check password if not locked
                if ($password === $db_password) {
                    ...
                    $update_stmt = $conn->prepare("UPDATE employees SET failed_attempts = ?, lockout_time = ? WHERE Email = ?");
